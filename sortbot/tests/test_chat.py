...
class BoomChatVLM(MockVLM):
    ...
    def chat(self, heard, context, overhead_jpeg=None, wrist_jpeg=None):
        raise AssertionError(f"the chat model was called for {heard!r} -- the regex pre-filter must handle it")
...
def test_directive_queue() -> None:
    ...
    assert [d["text"] for d in q.peek()] == ["reds on the left", "the blue one is behind the cup"]
    got = q.drain()
    assert [d["kind"] for d in got] == ["rule", "hint"] and q.drain() == [] and q.peek() == []
    t = m.Transcript(maxlen=3)
    for i in range(5):
        t.add("you" if i % 2 == 0 else "luna", f"line {i}")
    assert [e["text"] for e in t.entries()] == ["line 2", "line 3", "line 4"], t.entries()
...
def test_speak_priority() -> None:
    ...
    hold = threading.Event()      # a live stand-in for the TTS worker, so nothing drains q_say here
    ...
    try:
        ...
        assert v.pending_say() == ["planner line one", "planner line two"], v.pending_say()
...
def test_chat_worker() -> None:
    ...
    reply = "Sure - I'll keep the red ones on the left."
    script = [{"reply": reply, "rules": ["put red things on the left side of the table"],
               "hints": ["start with the red block"], "urgent": "none"}]
    vlms: list = []
    ...
    args = Namespace(max_steps=6, no_voice=True, hud_port=port, rules_file=str(tmp / "rules.json"), config=None)
    ...
    try:
        ...
        _wait(lambda: session.last_overhead is not None and session.last_wrist is not None,
        ...
        _wait(lambda: any(e["who"] == "luna" for e in get("/state")["chat"]["transcript"]), what="Luna replied")
        answered_in = time.time() - t0
        assert answered_in < 3.0, f"chat reply took {answered_in:.2f}s -- the loop must not be in the path"
        st = get("/state")
        tr = st["chat"]["transcript"]
        assert [e["who"] for e in tr] == ["you", "luna"], tr
        assert tr[0]["text"].startswith("hey Luna") and tr[1]["text"] == reply, tr
        assert st["chat"]["model"] == "mock" and st["chat"]["replies"] == 1 and st["chat"]["alive"]
        assert session.voice.last_said == reply, "the reply must go out on q_say"
        assert vlm.chat_calls and vlm.chat_calls[0]["images"] == 2, vlm.chat_calls  # cached overhead + wrist
        ...
        assert "put red things on the left side of the table" in get("/state")["voice"]["queue"]
        assert "put red things on the left side of the table" not in get("/state")["rules"]["list"]

        # the loop drains q_directives at its normal drain point and obeys
        assert post("step_once")["ok"]
        _wait(lambda: "put red things on the left side of the table" in get("/state")["rules"]["list"],
        ...
        assert Path(tmp / "rules.json").exists() and "put red things on the left side of the table" \
            in json.loads(Path(tmp / "rules.json").read_text()), "the rule must persist"
        ...
        _wait(lambda: session.ctl.stop_ev.is_set(), timeout=3.0, what="stop_ev set by the urgent path")
        ...
        assert [e["text"] for e in get("/state")["chat"]["transcript"]][-1] == "Stopping."
        _wait(lambda: get("/state")["run"]["phase"] in ("stopped", "done"), what="run stopped")
        ...
        session.chat.handle("hold on a second")
        ...
        _wait(lambda: any(d["kind"] == "cmd" and d["text"] == "open" for d in session.directives.peek()),
              what="bare command queued as a directive")
        assert "open" in get("/state")["voice"]["queue"]  # still "queued for the loop" on the page
        assert post("start")["ok"]  # a fresh run drains it -- under the bus lock, on the loop thread
        _wait(lambda: any(e["tool"] == "open" for e in get("/log")), what="the loop executed the bare command")
        post("stop")  # may already have finished on its own
        _wait(lambda: get("/state")["run"]["phase"] in ("stopped", "done"), what="run over")
        ...
        assert any(d["kind"] == "rule" for d in session.directives.peek()), session.directives.peek()

        # --- 5. the conversation never touched the bus (SORTBOT_BUS_ASSERT=1 is armed) -------------
        bad = [e for e in get("/log") if "BUS LOCK VIOLATION" in str(e.get("result", ""))]
    ...
    print(f"chat OK: rule obeyed by the loop, urgent stop in <1s with no model call, bare command via "
